...
const useNavigationIndex = false;
...
export class IQGeoHistoryManager {
        ...
        if (updateHistory) {
            ...
            if (index !== -1) {
                if (useNavigationIndex) {
                    ...
                    // Move the item to the end of the history
                    const item = this.navigationHistory.splice(index, 1)[0];
    ...
    async provideCodeLenses(doc) {
        ...
        if (doc.isDirty) {
            // Position the actions before the current function whilst editing to avoid too much movement of the actions.
    ...
    async _getHoverString(doc, pos) {
        ...
        if (currentText) {
            ...
            if (!currentText.includes('\n')) {
                ...
                const classData = this.iqgeoVSCode.getClassData(currentText, doc.languageId);
                if (classData) {
                    const searchCommand = vscode.Uri.parse(
                        `command:iqgeo.searchSymbols?${encodeURIComponent(
        ...
        if (historyString) {
            hoverString += `${lineSeparator}$(history)\u2002${historyString}`;
        }
    ...
    _getHistoryItems() {
        ...
        if (useNavigationIndex) {
            ...
            max = Math.min(this.navigationIndex + inc, this.navigationHistory.length - 1);
            const dec = max > this.navigationIndex ? 4 : 3;
            min = Math.max(max - dec, 0);
        } else {
            max = this.navigationHistory.length - 1;
            min = Math.max(max - 4, 0);
        }
